...
class RegistrationUserForm(FlaskForm):
    ...
    birth_date = DateField('Birth Date', validators=[DataRequired(), DateRange(max=date.today())])
...
class RegistrationDogForm(FlaskForm):
    ...
    #dog_temperament = StringField("What's his/her's temperament?", validators=[DataRequired(),Length(min=2,max=120)])
...
class EventForm(FlaskForm):
    ...
    date = DateField('Start Date', default=date.today(), validators=[DataRequired(), DateRange(min=date.today())])
    time = TimeField('Start Time', default=datetime.now().time(), validators=[DataRequired()])
